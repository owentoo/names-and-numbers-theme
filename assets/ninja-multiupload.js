...
/* Validate the order reference before submitting properties[design re-used].
   Real Shopify orders use the "NT-1234567" format. Earlier the API was
   returning a customer GraphQL ID (long hex like "a02f95a1d9d34d08c28954049be5c93c")
   which then leaked into the order admin as a meaningless string. Reject
   anything that doesn't look like an order name. Strips an optional
   "previously on order # " prefix so values from __reByUrl / __reByName
   maps (which already include the prefix) are validated correctly. */
...
/* --- design re-used attribution: canonical basename key ---
   Mirror of `designReUsedCanonicalKey()` in bySize_popular.js. The
   upload pipeline rewrites externally-fetched files to:
     `${nameOnly}____image_${Date.now()}_${Math.floor(Math.random()*1000)}.${ext}`
   so a raw basename match against the OLD filename misses after the
   URL-param (?file=...) re-upload triggered by My Designs cross-page
   navigation. Stripping the `____image_TS_RAND` suffix and applying
   the same alphanumeric-flatten that `nameOnly` uses gives a stable
   key across the round-trip. */
function designReUsedCanonicalKey( basename ) {
  if ( !basename ) return '';
...
.on(`click`, `product-form .addToCartGroupItems`,async function( e ) {
   ...
   var precutqty = pcutqty;
    ...
    const getAllProperties = getFormProperties( formEle, ['Remove Background', 'Super Resolution', 'Precut', '_discount_input', '_discount_name',  'Upload (Vector Files Preferred)', '_Original Image', 'design re-used'] );
    ...
    if ( typeof isAIImage !== 'undefined' && isAIImage ) {
      ...
      if(typeof properties['Upload (Vector Files Preferred)'] != "undefined" && typeof properties['_Original Image'] != "undefined"  && properties['_Original Image'].indexOf("transparent.png") == -1  && proccessNext == true){
        if(properties['Upload (Vector Files Preferred)'].indexOf("https") > -1){
          /* "design re-used" attribution for designs picked from
             /pages/my-designs purchases tab. The bridge in
             product-page-popups{,-multiupload}.liquid populates
             window.__designReUsedByName (basename → "previously on
             order # NT-…") plus a URL fallback map. Basename matching
             is the robust primary path because the upload pipeline
             rewrites the URL host (imgix ↔ S3) and adds processing
             query params (?trim=…&bg-remove=…) between the picker
             fire and cart serialization, while the basename
             survives every transformation we've observed. Final
             fallback to legacy window.grt_photo_id keeps any path
             that still sets that global working. */
          try {
            var __reByUrl  = (typeof window !== 'undefined' && window.__designReUsedByUrl)  || {};
            var __reByName = (typeof window !== 'undefined' && window.__designReUsedByName) || {};
            var __up   = properties['Upload (Vector Files Preferred)'] || '';
            var __orig = properties['_Original Image'] || '';
            var __urlProbe = [
              __up, __orig,
              __up.split('?')[0], __orig.split('?')[0]
            ];
            if (typeof ninjaImgixHost !== 'undefined' && typeof ninjaS3Host2 !== 'undefined') {
              __urlProbe.push(__up.split(ninjaImgixHost).join(ninjaS3Host2).split('?')[0]);
              __urlProbe.push(__up.split(ninjaS3Host2).join(ninjaImgixHost).split('?')[0]);
            }
            if (typeof ninjaS3Host !== 'undefined' && typeof ninjaS3Host2 !== 'undefined') {
              __urlProbe.push(__up.split(ninjaS3Host2).join(ninjaS3Host).split('?')[0]);
              __urlProbe.push(__up.split(ninjaS3Host).join(ninjaS3Host2).split('?')[0]);
            }
            for (var __pi = 0; __pi < __urlProbe.length; __pi++) {
              var __k = __urlProbe[__pi];
              if (__k && __reByUrl[__k] && _ntIsValidOrderRef(__reByUrl[__k])) {
                properties['design re-used'] = __reByUrl[__k];
                break;
              }
            }
            if (!properties['design re-used']) {
              var __nameProbe = [
                __up.split('?')[0].split('/').pop(),
                __orig.split('?')[0].split('/').pop()
              ];
              for (var __ni = 0; __ni < __nameProbe.length; __ni++) {
                var __nk = __nameProbe[__ni];
                if (__nk && __reByName[__nk] && _ntIsValidOrderRef(__reByName[__nk])) {
                  properties['design re-used'] = __reByName[__nk];
                  break;
                }
              }
              /* Canonical-key fallback: handles the URL-param re-upload
                 case where the cart filename has a fresh
                 `____image_TS_RAND.ext` suffix that the stash side
                 couldn't predict. */
              if (!properties['design re-used']) {
                for (var __cni = 0; __cni < __nameProbe.length; __cni++) {
                  var __cnk = designReUsedCanonicalKey(__nameProbe[__cni]);
                  if (__cnk && __reByName[__cnk] && _ntIsValidOrderRef(__reByName[__cnk])) {
                    properties['design re-used'] = __reByName[__cnk];
                    break;
                  }
                }
              }
            ...
            if (!properties['design re-used'] &&
                typeof window.grt_photo_id === 'string' &&
                window.grt_photo_id &&
                window.grt_photo_id.indexOf('previously on order #') === 0 &&
                _ntIsValidOrderRef(window.grt_photo_id)) {
              properties['design re-used'] = window.grt_photo_id;
            }
            if (!properties['design re-used'] &&
                typeof window.ordername === 'string' &&
                window.ordername &&
                _ntIsValidOrderRef(window.ordername)) {
              properties['design re-used'] = 'previously on order # ' + window.ordername;
            }
          } catch(__e) {}
 ...
 try {  } catch ( err ) {
   ...
   $(`.multiupload-files > li`).each(function(j) {
    ...
    const getAllProperties = getFormProperties( $( `#${_id}`), ['Design Notes','Remove Background', 'Super Resolution', 'Precut', '_discount_input', '_discount_name',  'Upload (Vector Files Preferred)', '_Original Image', 'design re-used'] );
    ...
    if(typeof properties['_Size'] != "undefined"){
      ...
      if(typeof properties['Upload (Vector Files Preferred)'] != "undefined" && typeof properties['_Original Image'] != "undefined"  && properties['_Original Image'].indexOf("transparent.png") == -1  && proccessNext == true){
        if(properties['Upload (Vector Files Preferred)'].indexOf("https") > -1){
          /* See the matching block in the addToCartGroupItems handler
             above for full rationale. Basename match is the robust
             primary path because the upload pipeline rewrites host /
             query params between picker fire and cart serialization. */
          try {
            var __reByUrl  = (typeof window !== 'undefined' && window.__designReUsedByUrl)  || {};
            var __reByName = (typeof window !== 'undefined' && window.__designReUsedByName) || {};
            var __up   = properties['Upload (Vector Files Preferred)'] || '';
