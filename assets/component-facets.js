...
class FacetFiltersForm extends HTMLElement {
  ...
  static renderSectionFromFetch(url, event) {
    fetch(url)
      ...
      .then((responseText) => {
        ...
        FacetFiltersForm.renderBhSidebar(html);
      });
  }
  ...
  static renderBhSidebar(html) {
    const parsedHTML = new DOMParser().parseFromString(html, 'text/html');
    const newPanel = parsedHTML.querySelector('.bh-sidebar-panel');
    const currentPanel = document.querySelector('.bh-sidebar-panel');
    if (newPanel && currentPanel) {
      currentPanel.innerHTML = newPanel.innerHTML;
      /* Re-attach "Show all" toggle listeners */
      currentPanel.querySelectorAll('.bh-sb-show-all').forEach(function(btn) {
      ...
      /* Re-attach active filter chip removal via AJAX */
      currentPanel.querySelectorAll('.js-facet-remove').forEach(function(link) {
        link.addEventListener('click', function(e) {
          e.preventDefault();
          var href = link.getAttribute('href');
          var searchParams = href.indexOf('?') === -1 ? '' : href.slice(href.indexOf('?') + 1);
  ...
  onSubmitHandler(event) {
    ...
    const eventFormId = event.target.closest('form').id;
    const eventIsSortForm = (eventFormId === 'FacetSortForm' || eventFormId === 'BhMobileSortForm');

    sortFilterForms.forEach((form) => {
      if (!isMobile) {
        const isSortForm = (form.id === 'FacetSortForm' || form.id === 'BhMobileSortForm');
        /* If event came from a sort form, only include THAT sort form (skip the other) */
        if (eventIsSortForm && isSortForm && form.id !== eventFormId) return;
        /* If event came from a filter, skip whichever sort form is hidden */
        if (!eventIsSortForm && isSortForm && !form.offsetParent) return;
        if (form.id === 'FacetSortForm' || form.id === 'BhMobileSortForm' || form.id === 'FacetFiltersForm' || form.id === 'FacetSortDrawerForm' || form.classList.contains('bh-sb-form')) {
...
/* ============================================================
   BLANKS MOBILE FILTER DRAWER
   Moves .bh-sidebar-panel into drawer on open, back on close.
   Moves sort <select> into mobile toolbar on small screens.
   ============================================================ */
(function(){
  var drawer = document.getElementById('blanks-filter-drawer');
  var drawerBody = document.getElementById('blanks-filter-drawer-body');
  var sidebar = document.querySelector('.bh-blanks-sidebar');
  if (!drawer || !drawerBody || !sidebar) return;

  var panel = sidebar.querySelector('.bh-sidebar-panel');
  if (!panel) return;

  /* Open drawer: move panel in, then show */
  window.__bhOpenFilterDrawer = function(){
    drawerBody.appendChild(panel);
    drawer.show();
  };

  /* Close helper: hide drawer + move panel back */
  function closeDrawer(){
    drawer.hide();
    setTimeout(function(){ sidebar.appendChild(panel); }, 520);
  }

  /* On close button: move panel back to sidebar after transition */
  ...
  /* Auto-close drawer when a filter checkbox/swatch is clicked */
  drawerBody.addEventListener('change', function(e){
    var target = e.target;
    if (target.matches('input[type="checkbox"]')) {
      /* Ensure input event fires for AJAX (some mobile browsers only fire change) */
      target.dispatchEvent(new Event('input', {bubbles: true}));
      /* Close drawer after a short delay so user sees the selection */
      setTimeout(closeDrawer, 400);
    }
  });

  /* Swap product card images to match active color filter */
  function bhSwapColorImages() {
    var params = new URLSearchParams(window.location.search);
    var activeColors = params.getAll('filter.p.m.custom.facet_colors');
    if (!activeColors.length) return;

    var lowerColors = activeColors.map(function(c){ return c.toLowerCase(); });

    var cards = document.querySelectorAll('#CollectionProductGrid .product-item');
    cards.forEach(function(card) {
      /* Parse the color→image map (variant color name → image URL) */
      var mapEl = card.querySelector('script.bh-color-map');
      var colorMap = null;
      if (mapEl) {
        try { colorMap = JSON.parse(mapEl.textContent); } catch(e) { colorMap = null; }
      }

      var matchImg = null;

      /* Strategy 1: Use ColorVariantLookup metafield to map filter color → first variant name → image */
      var lookupEl = card.querySelector('script.bh-color-lookup');
      if (lookupEl && colorMap) {
        try {
          var lookup = JSON.parse(lookupEl.textContent);
          for (var j = 0; j < lowerColors.length; j++) {
            /* Find the matching key in the lookup (case-insensitive) */
            var lookupKeys = Object.keys(lookup);
            for (var lk = 0; lk < lookupKeys.length; lk++) {
              if (lookupKeys[lk].toLowerCase() === lowerColors[j]) {
                var variants = lookup[lookupKeys[lk]];
                if (variants && variants.length) {
                  /* Try each variant name in order until we find one with an image */
                  for (var vi = 0; vi < variants.length; vi++) {
                    var varName = variants[vi].toLowerCase();
                    if (colorMap[varName]) {
                      matchImg = colorMap[varName];
                      break;
                    }
                  }
                }
      ...
      /* Strategy 2: Substring match on color map keys (original fallback) */
      if (!matchImg && colorMap) {
        var colorKeys = Object.keys(colorMap);
        for (var j = 0; j < lowerColors.length; j++) {
          for (var k = 0; k < colorKeys.length; k++) {
            if (colorKeys[k].indexOf(lowerColors[j]) !== -1) {
              matchImg = colorMap[colorKeys[k]];
              break;
            }
          }
          if (matchImg) break;
      ...
      /* Strategy 3: Check rendered swatches */
      if (!matchImg) {
        var swatches = card.querySelectorAll('.bh-sw[data-img][data-color]');
        for (var i = 0; i < swatches.length; i++) {
          var swColor = (swatches[i].dataset.color || '').toLowerCase();
          for (var j = 0; j < lowerColors.length; j++) {
            if (swColor.indexOf(lowerColors[j]) !== -1) {
              matchImg = swatches[i].dataset.img;
              break;
            }
          }
          if (matchImg) break;
      ...
      /* Swap main image */
      var imgEl = card.querySelector('.product-item__image-figure img.img');
      if (imgEl) {
  ...
  /* After AJAX re-render: swap color images + mobile badge/sort sync */
  var grid = document.getElementById('CollectionProductGrid');
  if (grid) {
    var observer = new MutationObserver(function(){
      /* Color image swap — runs on ALL viewports */
      bhSwapColorImages();

      /* Mobile-only: sort sync + badge */
      if (window.innerWidth < 768) {
        /* Sync mobile sort select with the server-rendered value */
        var desktopSort = grid.querySelector('#FacetSortForm .bh-grid-sort__select');
        var mobileSort = document.querySelector('#BhMobileSortForm .bh-grid-sort__select');
        if (desktopSort && mobileSort && mobileSort.value !== desktopSort.value) {
          mobileSort.value = desktopSort.value;
        }

        /* Update filter badge count */
        var countEl = document.getElementById('bh-active-filter-count');
        var badge = document.querySelector('.bh-mobile-toolbar__badge');
        var btn = document.querySelector('.bh-mobile-toolbar__filter');
        if (countEl && btn) {
          var c = parseInt(countEl.dataset.count, 10) || 0;
          if (c > 0) {
            if (!badge) {
              badge = document.createElement('span');
              badge.className = 'bh-mobile-toolbar__badge';
